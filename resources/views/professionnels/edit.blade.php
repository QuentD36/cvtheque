...
@section("contenu")
    ...
    <div class="card shadow mb-4">
        ...
        <!-- Card Body -->
        <div class="card-body">
            <form method="post" action="{{route('professionnels.update', $professionnel->id)}}">
                @method('PUT')
                @csrf
                ...
                <div class="form-group col-xl-12 col-perso">
                    <select class="custom-select select-perso @error('metier_id') is-invalid @enderror" name="metier_id">
                        <option value="" @if(old('metier_id') == '' || $professionnel->metier_id == '') selected @endif>
                            Choix du métier
                        </option>
                        @foreach($metiers as $metier)
                            <option value="{{$metier->id}}" @if(old('metier_id') == $metier->id || $professionnel->metier_id == $metier->id) selected @endif>
                                {{$metier->libelle}}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <div class="radio-container offset-xl-2">
                        ...
                        <div class="radio-content custom-radio">
                            <input class="custom-radio" type="radio" value="1" name="formation" @if(old('formation', $professionnel->formation) == 1) checked @endif>
                            <label for="oui">Oui</label>
                            <input class="custom-radio" type="radio" value="0" name="formation"  @if(old('formation', $professionnel->formation) == 0) checked @endif>
                ...
                <div class="form-group col-xl-12 col-perso">
                    <input type="text" class="input-perso form-control form-control-user @error('nom') is-invalid border-danger @enderror"
                           placeholder="Nom" value="{{old('nom', $professionnel->nom)}}" name="nom">
                    @error('nom')
                    <p class="alert-danger alert-danger-perso p-perso" role="alert">{{$message}}</p>
                    @enderror
                    <input type="text" class="input-perso form-control form-control-user @error('prenom') is-invalid border-danger @enderror"
                           placeholder="Prénom" value="{{old('prenom', $professionnel->prenom)}}" name="prenom">
                    @error('prenom')
                    <p class="alert-danger alert-danger-perso p-perso" role="alert">{{$message}}</p>
                    @enderror
                </div>
                <div class="form-group col-xl-12 col-perso">
                    <input @if(old('naissance') != '' || $professionnel->naissance != '') type="date" @else type="text" @endif class="input-perso form-control form-control-user @error('naissance') is-invalid border-danger @enderror"
                           placeholder="Date de naissance" value="{{old('naissance', $professionnel->naissance)}}" name="naissance" onfocus="(this.type='date')">
                    @error('naissance')
                    <p class="alert-danger alert-danger-perso p-perso" role="alert">{{$message}}</p>
                    @enderror
                </div>
                <div class="form-group col-xl-12 col-perso">
                    <input type="text" class="input-perso form-control form-control-user @error('cp') is-invalid border-danger @enderror"
                           placeholder="Code postal" value="{{old('cp', $professionnel->cp)}}" name="cp">
                    @error('cp')
                    <p class="alert-danger alert-danger-perso p-perso" role="alert">{{$message}}</p>
                    @enderror
                    <input type="text" class="input-perso form-control form-control-user @error('ville') is-invalid border-danger @enderror"
                           placeholder="Ville" value="{{old('ville', $professionnel->ville)}}" name="ville">
                    @error('ville')
                    <p class="alert-danger alert-danger-perso p-perso" role="alert">{{$message}}</p>
                    @enderror
                </div>
                <div class="form-group col-xl-12 col-perso">
                    <input type="tel" class="input-perso form-control form-control-user @error('telephone') is-invalid border-danger @enderror"
                           placeholder="Téléphone" value="{{old('telephone', $professionnel->telephone)}}" name="telephone">
                    @error('telephone')
                    <p class="alert-danger alert-danger-perso p-perso" role="alert">{{$message}}</p>
                    @enderror
                    <input type="text" class="input-perso form-control form-control-user @error('email') is-invalid border-danger @enderror"
                           placeholder="E-mail" value="{{old('email', $professionnel->email)}}" name="email">
                    @error('email')
                    <p class="alert-danger alert-danger-perso p-perso" role="alert">{{$message}}</p>
                    @enderror
                </div>
                <div class="form-group col-xl-12 col-perso">
                    <div class="checkbox-container" >
                        ...
                        <input type="checkbox" id="reseaux" name="domaine[]" value="R" {{(is_array(old('domaine', $professionnel->domaine)) && in_array('R', old('domaine', $professionnel->domaine))) ? 'checked' : ''}}>
                        <label for="systemes">Systèmes</label>
                        <input type="checkbox" id="systemes" name="domaine[]" value="S" {{(is_array(old('domaine', $professionnel->domaine)) && in_array('S', old('domaine', $professionnel->domaine))) ? 'checked' : ''}}>
                        <label for="developpement">Développement</label>
                        <input type="checkbox" id="developpement" name="domaine[]" value="D" {{(is_array(old('domaine', $professionnel->domaine)) && in_array('D', old('domaine', $professionnel->domaine))) ? 'checked' : ''}}>
                    </div>
                    @error('domaine')
                    <p class="alert-danger alert-danger-perso p-perso" role="alert">{{$message}}</p>
                    @enderror
                ...
                <div class="form-group col-xl-12 col-perso">
                    <input type="text" class="input-perso form-control form-control-user @error('source') is-invalid border-danger @enderror"
                           placeholder="Source" value="{{old('source', $professionnel->source)}}" name="source">
                    @error('source')
                    <p class="alert-danger alert-danger-perso p-perso" role="alert">{{$message}}</p>
                    @enderror
                ...
                <div class="form-group col-xl-12 col-perso">
                    <textarea class="textarea-perso form-control form-control-user @error('observation') is-invalid border-danger @enderror" rows="5" cols="50" name="observation" placeholder="Observation">{{old('observation', $professionnel->observation)}}</textarea>
                    @error('observation')
                    <p class="alert-danger alert-danger-perso p-perso" role="alert">{{$message}}</p>
                    @enderror
                </div>
